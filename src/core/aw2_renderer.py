...
from src.core.aw2_data import TERRAIN_ID_TO_SPRITE
...
class AW2Renderer:
    ...
    def _get_sprite_for_terrain(self, terrain_id: int) -> np.ndarray:
        ...
        if sprite_name is None:
            logger.debug(f"Unknown terrain ID: {terrain_id}")
            return self._fallback_sprite
        
        sprite = self.atlas.get(sprite_name)
        if sprite is None:
            logger.debug(f"Sprite not found: {sprite_name} for terrain ID {terrain_id}")
            return self._fallback_sprite
    ...
    def render_map(self, map_data: Dict[str, Any]) -> Tuple[bool, io.BytesIO]:
        ...
        Args:
            map_data: Map data from API containing size_w, size_h, terr
        ...
        if terrain_ids.shape != (height, width):
            logger.warning(f"Terrain shape {terrain_ids.shape} doesn't match map size {height}x{width}")
            terrain_ids = terrain_ids[:height, :width]
        
        # Render map image
        img = self._render(terrain_ids, width, height)
    ...
    def _render(self, terrain_ids: np.ndarray, width: int, height: int) -> Image.Image:
        ...
        for y in range(height):
            for x in range(width):
                ...
                # Alpha-composite the sprite on top (for ALL tiles, not just properties)
                # Use alpha channel as mask for proper transparency blending
