...
async def download_image(url: str, file_path: Union[str, BytesIO]) -> None:
    is_path = False
    ...
    try:
        async with ClientSession() as session:
            async with session.get(url) as resp:
                if resp.status != 200:
                    raise IOError(f"Failed to download image: {resp.status}")
                while True:
                    chunk = await resp.content.read(1024) # Increased chunk size
                    if not chunk:
                        break
                    fd.write(chunk)
